...
(async () => {
  try {
    ...
    execSync(`node scripts/make-iconsax.js --outdir "${outdir}"`, { stdio: 'inherit' });
    execSync(`node scripts/make-fonts.js --outdir "${outdir}"`, { stdio: 'inherit' });
    execSync(`node scripts/make-assets.js --outdir "${outdir}"`, { stdio: 'inherit' });
    execSync(`node scripts/make-flags.js --outdir "${outdir}"`, { stdio: 'inherit' });
    if (types) {
  ...
  if (serve) {
    ...
    bs.init(browserSyncConfig, () => {
      ...
      setTimeout(() => {
        ...
        console.log(chalk.cyan(`Launched the em9-Library dev server at ${url} 🥾\n`));
