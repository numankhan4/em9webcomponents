...
(() => {
  ...
  window.$docsify.plugins.push(hook => {
    ...
    hook.mounted(() => {
      ...
      searchBox.innerHTML = `
        <em9-input
          ...
          pill
        >
          <em9-icon slot="prefix" name="search"></em9-icon>
          <kbd slot="suffix" title="Press / to search">/</kbd>
        </em9-input>
      `;
      const searchBoxInput = searchBox.querySelector('em9-input');
        ...
        matches.forEach((match, index) => {
          ...
          a.innerHTML = `
            <div class="site-search__result-icon">
              <em9-icon name="${icon}" aria-hidden="true"></em9-icon>
            </div>
    ...
    input.addEventListener('em9-input', handleInput);
