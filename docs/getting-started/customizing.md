...
em9-Library components can be customized at a high level through design tokens. This gives you control over theme colors and general styling. For more advanced customizations, you can make use of component parts and custom properties to target individual components.

## Design Tokens

em9-Library makes use of several design tokens to provide a consistent appearance across components. You can customize them and use them in your own application with pure CSS — no preprocessor required.
...
Design tokens are accessed through CSS custom properties that are defined in your theme. Because design tokens live at the page level, they're prefixed with `--em9-` to avoid collisions with other libraries.
...
:root {
  ...
  --em9-color-primary-50: var(--em9-color-purple-50);
  --em9-color-primary-100: var(--em9-color-purple-100);
  --em9-color-primary-200: var(--em9-color-purple-200);
  --em9-color-primary-300: var(--em9-color-purple-300);
  --em9-color-primary-400: var(--em9-color-purple-400);
  --em9-color-primary-500: var(--em9-color-purple-500);
  --em9-color-primary-600: var(--em9-color-purple-600);
  --em9-color-primary-700: var(--em9-color-purple-700);
  --em9-color-primary-800: var(--em9-color-purple-800);
  --em9-color-primary-900: var(--em9-color-purple-900);
  --em9-color-primary-950: var(--em9-color-purple-950);
}
```

Many design tokens are described further along in this documentation. For a complete list, refer to `src/themes/light.css` in the project's [source code](https://github.com/em9-style/em9/blob/current/src/themes/light.css).
...
```html preview
<em9-button class="tomato-button"> Tomato Button </em9-button>
...
For convenience, some components expose CSS custom properties you can override. These are not design tokens, nor do they have the same `--em9-` prefix since they're scoped to a component.
...
```html
<em9-avatar style="--size: 6rem;"></em9-avatar>
```
...
```js
import { setDefaultAnimation } from 'em9webcomponents/dist/utilities/animation-registry.js';
...
```js
import { setAnimation } from 'em9webcomponents/dist/utilities/animation-registry.js';
