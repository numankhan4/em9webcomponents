...
import type em9ImageComparer from './image-comparer';

describe('<em9-image-comparer>', () => {
  it('should render a basic before/after', async () => {
    const el = await fixture<em9ImageComparer>(html`
      <em9-image-comparer>
        <div slot="before"></div>
        <div slot="after"></div>
      </em9-image-comparer>
  ...
  it('should emit change event when position changed manually', async () => {
    ...
    el.addEventListener('em9-change', handler, { once: true });
  ...
  it('should set position by attribute', async () => {
    const el = await fixture<em9ImageComparer>(html`
      <em9-image-comparer position="10">
        <div slot="before"></div>
        <div slot="after"></div>
      </em9-image-comparer>
    `);
