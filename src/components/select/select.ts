...
import type em9Dropdown from '../../components/dropdown/dropdown';
import type em9IconButton from '../../components/icon-button/icon-button';
import type em9MenuItem from '../../components/menu-item/menu-item';
import type em9Menu from '../../components/menu/menu';
 ...
 * @event em9-change - Emitted when the control's value changes.
 ...
 * @csspart menu - The select menu, an `<em9-menu>` element.
 * @csspart tag - The multi select option, an `<em9-tag>` element.
 ...
 */
@customElement('em9-select')
export default class em9Select extends LitElement {
  static styles = styles;

  @query('.select') dropdown: em9Dropdown;
  @query('.select__control') control: em9Dropdown;
  @query('.select__hidden-select') input: HTMLInputElement;
  @query('.select__menu') menu: em9Menu;

  // @ts-expect-error -- Controller is currently unused
  private readonly formSubmitController = new FormSubmitController(this);
  private readonly hasSlotController = new HasSlotController(this, 'help-text', 'label');
  private readonly localize = new LocalizeController(this);
  private menuItems: em9MenuItem[] = [];
  ...
  /** Enables search field. With this enabled, display label will be hide. */
  @property({ type: Boolean, reflect: true }) searchFilter = false;

  /** Enables multi select. With this enabled, value will be an array. */
  @property() searchFilterClass = '';
  ...
  /** Connect select with Input. */
  @property({type: Boolean}) connector = false;



  /** The selects placeholder text. */
  ...
  @property({type: Boolean, reflect: true, attribute: 'label-content',}) displayLabelWithContent = false;
  ...
  handleKeyDown(event: KeyboardEvent) {
    ...
    if (target.tagName.toLowerCase() === 'em9-tag') {
      return;
    }
  ...
  async handleMenuSlotChange() {
    ...
    this.menuItems = [...this.querySelectorAll<em9MenuItem>('em9-menu-item')];

    // add class to menuItems to style them


    this.menuItems.forEach(item => {
      item.shadowRoot?.querySelectorAll('.menu-item').forEach(el => {
        el.classList.add(`menu-item-${this.size}`);
      });
    });
    ...
    this.menuItems.forEach(item => {
      if (values.includes(item.value)) {
        console.error(`Duplicate value found in <em9-select> menu item: '${item.value}'`, item);
      }
  ...
  handleTagInteraction(event: KeyboardEvent | MouseEvent) {
    ...
    const clearButton = path.find((el: em9IconButton) => {
  ...
  syncItemsFromValue() {
    ...
    if (this.multiple) {
      ...
      this.displayTags = checkedItems.map((item: em9MenuItem) => {
        return html`
          <em9-tag
            part="tag"
            exportparts="
      ...
      >
        <label
          ...
          class=${classMap({
            ...
            'form-control__label-line': this.lineStyled
          })}
          for="input"
        ...
        <div part="form-control-input" class="form-control-input">
          <em9-dropdown
            ...
            class=${classMap({
              ...
              'select--line': this.lineStyled,
              'select--small': this.size === 'small',
              'select--medium': this.size === 'medium',
              'select--large': this.size === 'large',
              'select--pill': this.pill,
              'select--invalid': this.invalid,
              'connector--select': this.connector
            })}
            @em9-show=${this.handleMenuShow}
            @em9-hide=${this.handleMenuHide}
          >
            <div
              part="control"
              slot="trigger"
              ...
              ${ this.searchFilter
                ? html`
                  <div part="display-label" class="select__filter">
                    <input
                ...
                ? html`
                  ...
                  >
                    <slot name="clear-icon">
                      <em9-icon name="x-circle-fill" library="system"></em9-icon>
                    </slot>
                  </button>
                `
                : ''}
              ...
              <span part="icon" class="select__icon" aria-hidden="true">
                <em9-icon name="cs-linear-chevron-down"></em9-icon>
              </span>
            ...
            <em9-menu part="menu" id="menu"
                      class="${classMap({
                        'select__menu': true,
                        'connector--menu': this.connector
                      })}"
                      @em9-select=${this.handleMenuSelect}>
              <slot @slotchange=${this.handleMenuSlotChange} @em9-label-change=${this.handleMenuItemLabelChange}></slot>
            </em9-menu>
          </em9-dropdown>
        </div>
