 ...
 * @slot copy - A button that copies the input's value to the clipboard. This slot is only available when the type is "text".
 *
 ...
 * @csspart symbol - The input sumbol container.
 */
@customElement("em9-input")
export default class em9Input extends LitElement {
  ...
  /** Copy To Clipboard to the input's value. */
  @property({ type: Boolean }) copy: boolean;

  /** prefix_label To render static value. */
  @property() prefix_label: string;

  /** prefix_label To render static value. */
  @property() suffix_label: string;
  ...
  handelCopyToClipboard() {
    const ctc = this.input;
    ctc.select();
    ctc.setSelectionRange(0, 99999); // For mobile devices

    if (window.isSecureContext && navigator.clipboard) {
      navigator.clipboard.writeText(ctc.value);
    } else {
      document.execCommand("copy");
    }

    this.invalid = ctc.value === "";
  }
  ...
  render() {
    ...
    const hasStaticLabel = !!this.prefix_label || !!this.suffix_label && this.size;
    const hasPreFixLabel = !!this.prefix_label;
    const hasSufFixLabel = !!this.suffix_label;
    const hasCopyToClipboard = !!this.copy;
    ...
    return html`
      ...
      >
        <label
          ...
          for="input_${ifDefined(this.name)}"
          aria-hidden="${hasLabel ? "false" : "true"}"
        >
          <slot name="label">${this.label}${this.required ?html`<em9-icon part="required" name="cs-bold-indicator"></em9-icon>` : ''}</slot>
        </label>

        <div part="form-control-input"
             class="${classMap({
               "form-control-input": true,
               "has-static-label": hasStaticLabel,
               "has--prefix--labels": hasPreFixLabel,
               "has--suffix--labels": hasSufFixLabel,
               "has-copy-to-clipboard": hasCopyToClipboard,
               "suffix--line": this.lineStyled,
               "suffix--pill": this.pill,
               "suffix--standard": !this.filled && !this.lineStyled,
               "suffix--filled": this.filled,
               "suffix--disabled": this.disabled,
               "suffix--focused": this.hasFocus,
               "suffix--empty": !this.value,
               "suffix--invalid": this.invalid
             })}">
          ${this.prefix_label
            ? html`
              <span part="label-prefix" class="input_label_prefix">
                  <slot name="label_prefix">${this.prefix_label}</slot>
                </span>
            ` : html``
          }
          <div
            ...
            class="${classMap({
              ...
              // Sizes
              "input--line": this.lineStyled,
              "border--less": this.borderLess,
          ...
          >
                  ...
                  ${this.isPasswordVisible
                    ? html`
                      <slot name="show-password-icon">
                        <em9-icon name="eye-slash" library="system"></em9-icon>
                      </slot>
                    `
                    : html`
                      <slot name="hide-password-icon">
                        <em9-icon name="eye" library="system"></em9-icon>
                      </slot>
                    `}
                </button>
              `
            ...
            <span
              ...
              class="${classMap({
                input__suffix: true,
                "input__suffix-invalid": this.invalid
              })}">
              <slot name="suffix"></slot>
              <slot name="label_suffix"></slot>
            </span>
          ...
          ${this.suffix_label
            ? html`
              <span part="label-prefix" class="input_label_suffix">
          ...
          ${this.copy
            ? html`
              <em9-tooltip hoist content="Copy Input Value">
                <em9-button variant="link gray"
                            class="${classMap({
                              "input-copy-label": true,
                              "input-copy-invalid": this.invalid
                            })}"
                            size="${this.size}"
                            @click="${this.handelCopyToClipboard}">Copy
                </em9-button>
              </em9-tooltip>`
            : html``}
        </div>
        ...
        <div
          ...
          class="${classMap({
            ...
            "form-control__help-text-invalid": this.invalid
          })}"
