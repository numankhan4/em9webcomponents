...
import type em9Avatar from './avatar';
...
describe('<em9-avatar>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<em9Avatar>(html` <em9-avatar label="Avatar"></em9-avatar> `);
    });
  ...
  describe('when provided an image and label parameter', () => {
    ...
    before(async () => {
      el = await fixture<em9Avatar>(html`<em9-avatar image="${image}" label="${label}"></em9-avatar>`);
    });

    it('should pass accessibility tests', async () => {
      /**
       * The image element itself is ancillary, because it's parent container contains the
       * aria-label which dictates what "em9-avatar" is. This also implies that label text will
  ...
  describe('when provided initials parameter', () => {
    const initials = 'SL';
    before(async () => {
      el = await fixture<em9Avatar>(html`<em9-avatar initials="${initials}" label="Avatar"></em9-avatar>`);
    });
  ...
  ['square', 'rounded', 'circle'].forEach(shape => {
    describe(`when passed a shape attribute ${shape}`, () => {
      before(async () => {
        el = await fixture<em9Avatar>(html`<em9-avatar shape="${shape}" label="Shaped avatar"></em9-avatar>`);
      });
  ...
  describe('when passed a <span>, on slot "icon"', () => {
    before(async () => {
      el = await fixture<em9Avatar>(html`<em9-avatar label="Avatar"><span slot="icon">random content</span></em9-avatar>`);
    });
  ...
  it('should not render the image when the image fails to load', async () => {
    ...
    el = await fixture<em9Avatar>(html`<em9-avatar></em9-avatar>`);
