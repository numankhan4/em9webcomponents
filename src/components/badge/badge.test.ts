...
import type em9Badge from './badge';
...
describe('<em9-badge>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<em9Badge>(html` <em9-badge>Badge</em9-badge> `);
    });
  ...
  describe('when provided a pill parameter', () => {
    before(async () => {
      el = await fixture<em9Badge>(html` <em9-badge pill>Badge</em9-badge> `);
    });
  ...
  describe('when provided a pulse parameter', () => {
    before(async () => {
      el = await fixture<em9Badge>(html` <em9-badge pulse>Badge</em9-badge> `);
    });
  ...
  ['primary', 'success', 'neutral', 'warning', 'danger'].forEach(variant => {
    describe(`when passed a variant attribute ${variant}`, () => {
      before(async () => {
        el = await fixture<em9Badge>(html`<em9-badge variant="${variant}">Badge</em9-badge>`);
      });
