...
import type em9TabPanel from '../../components/tab-panel/tab-panel';
import type em9Tab from '../../components/tab/tab';
 ...
 *
 * @event {{ name: String }} em9-tab-show - Emitted when a tab is shown.
 * @event {{ name: String }} em9-tab-hide - Emitted when a tab is hidden.
 *
 ...
 */
@customElement('em9-tab-group')
export default class em9TabGroup extends LitElement {
  ...
  private activeTab?: em9Tab;
  private mutationObserver: MutationObserver;
  private resizeObserver: ResizeObserver;
  private tabs: em9Tab[] = [];
  private panels: em9TabPanel[] = [];
  ...
  getAllTabs(includeDisabled = false) {
    ...
    return [...(slot.assignedElements() as em9Tab[])].filter(el => {
      return includeDisabled
        ? el.tagName.toLowerCase() === 'em9-tab'
        : el.tagName.toLowerCase() === 'em9-tab' && !el.disabled;
    });
  }

  getAllPanels() {
    const slot = this.body.querySelector('slot')!;
    return [...slot.assignedElements()].filter(el => el.tagName.toLowerCase() === 'em9-tab-panel') as [em9TabPanel];
  }
  ...
  handleClick(event: MouseEvent) {
    ...
    const tab = target.closest('em9-tab');
  ...
  handleKeyDown(event: KeyboardEvent) {
    ...
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
      ...
      if (activeEl?.tagName.toLowerCase() === 'em9-tab') {
        let index = this.tabs.indexOf(activeEl as em9Tab);
  ...
  setActiveTab(tab: em9Tab, options?: { emitEvents?: boolean; scrollBehavior?: 'auto' | 'smooth' }) {
    options = {
      emitEvents: true,
      scrollBehavior: 'auto',
      ...options
    ...
    if (tab !== this.activeTab && !tab.disabled) {
      ...
      if (options.emitEvents) {
        if (previousTab) {
          emit(this, 'em9-tab-hide', { detail: { name: previousTab.panel } });
        }

        emit(this, 'em9-tab-show', { detail: { name: this.activeTab.panel } });
      }
    }
  }
