 ...
 *
 * @event {{ item: em9MenuItem }} em9-select - Emitted when a menu item is selected.
 *
 * @csspart base - The component's internal wrapper.
 */
@customElement('em9-menu')
export default class em9Menu extends LitElement {
  ...
  getAllItems(options: { includeDisabled: boolean } = { includeDisabled: true }) {
    return [...this.defaultSlot.assignedElements({ flatten: true })].filter((el: HTMLElement) => {
      ...
      if (!options.includeDisabled && (el as em9MenuItem).disabled) {
        return false;
      }
   ...
   */
  setCurrentItem(item: em9MenuItem) {
  ...
  typeToSelect(event: KeyboardEvent) {
    ...
    if (!hasFocusVisible) {
      items.forEach(item => item.classList.remove('em9-focus-invisible'));
    }
  ...
  handleClick(event: MouseEvent) {
    ...
    const item = target.closest('em9-menu-item');

    if (item?.disabled === false) {
      emit(this, 'em9-select', { detail: { item } });
    }
  }
  ...
  handleKeyUp() {
    ...
    if (!hasFocusVisible) {
      ...
      items.forEach(item => {
        item.classList.remove('em9-focus-invisible');
      });
    }
  }
  ...
  handleMouseDown(event: MouseEvent) {
    ...
    if (target.getAttribute('role') === 'menuitem') {
      this.setCurrentItem(target as em9MenuItem);

      // Hide focus in browsers that don't support :focus-visible when using the mouse
      if (!hasFocusVisible) {
        target.classList.add('em9-focus-invisible');
      }
    }
  }
