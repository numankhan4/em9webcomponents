 ...
 * @status stable
 *
 * @event em9-load - Emitted when the included file is loaded.
 * @event {{ status: number }} em9-error - Emitted when the included file fails to load due to an error.
 */
@customElement('em9-include')
export default class em9Include extends LitElement {
  ...
  async handleSrcChange() {
    try {
      ...
      if (!file.ok) {
        emit(this, 'em9-error', { detail: { status: file.status } });
        return;
      }
      ...
      emit(this, 'em9-error', { detail: { status: -1 } });
    }
  }
