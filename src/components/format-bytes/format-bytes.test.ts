...
import type em9FormatBytes from './format-bytes';

describe('<em9-format-bytes>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<em9FormatBytes>(html` <em9-format-bytes></em9-format-bytes> `);
  ...
  describe('bits', () => {
    ...
    results.forEach(expected => {
      it('bits : display formats', async () => {
        const el = await fixture<em9FormatBytes>(html` <em9-format-bytes unit="bit"></em9-format-bytes> `);
        // short
        el.value = expected.value;
        await elementUpdated(el);
        expect(el.shadowRoot?.textContent).to.equal(expected.short);
